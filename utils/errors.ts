...
const translations: FirebaseErrors = {
  ...
  "auth/requires-recent-login":
    "Você precisa sair e entrar novamente para alterar sua senha.",
...
export const translateFirebaseError = (error: FirebaseError): string => {
  if (!error || !error.code) {
    return "Ocorreu um erro inesperado.";
  }
  const { code } = error;
  const message = translations[code] || error.message;
  return message;
};
